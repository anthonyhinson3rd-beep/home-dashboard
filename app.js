...
function renderCalendar() {
  ...
  for (let i = 0; i < 28; i++) {
    ...
    const matches = events.filter(e => formatDateKey(e.date) === formatDateKey(date)).sort((a, b) => eventSortValue(a) - eventSortValue(b));
...
function renderAgenda() {
  ...
  const future = events.filter(event => event.date >= today).slice().sort((a, b) => (a.date - b.date) || (eventSortValue(a) - eventSortValue(b)));
...
function normalizeFeedEvent(item) {
  ...
  return { date, start: item.start || null, time: item.time || '', title: item.title || 'Untitled event', calendar: item.calendar || 'Other', color: item.color || null, allDay: Boolean(item.allDay), important: Boolean(item.important) };
}
...
async function loadCalendarFeed() {
  ...
  const keyDetected = key.length > 0;
...
setInterval(() => { renderCalendar(); renderAgenda(); }, 5 * 60 * 1000);
